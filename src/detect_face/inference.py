...
from src.detect_face.mtcnn_pytorch.src.get_nets import PNet, RNet, ONet
from src.detect_face.mtcnn_pytorch.src.box_utils import nms, calibrate_box, get_image_boxes, convert_to_square
from src.detect_face.mtcnn_pytorch.src.first_stage import run_first_stage
from src.detect_face.mtcnn_pytorch.src.align_trans import get_reference_facial_points, warp_and_crop_face
...
class MTCNN():
    ...
    def detect_faces(self, image, min_face_size=40.0,
        ...
        with torch.no_grad():
            ...
            bounding_boxes = bounding_boxes[keep]
            landmarks = landmarks[keep]
        return bounding_boxes.astype(int), landmarks
    ...
    def find_biggest_face(self,image):
        """
...
def load_detection_model_opencv(model_path):
    detection_model = cv2.CascadeClassifier(model_path)
    return detection_model

def detect_faces_opencv(detection_model, gray_image_array):
    return detection_model.detectMultiScale(gray_image_array, 1.3, 5)

def refine_bbox(bbox,h, w):
    """Restrict the bbox inside the image"""
    x1, y1, x2, y2, sc = bbox
    x1 = int(max(0, x1))
    y1 = int(max(0, y1))
    x2 = int(min(x2, w))
